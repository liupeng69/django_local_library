...
class LoanedBookInstancesByUserListViewTest(TestCase):
    def setUp(self):
        ...
        test_book = Book.objects.create(
            ...
            # genre = test_genre,  # WRONG!!! Direct assignment of many-to-many types not allowed.
        )
    ...
    def test_redirect_if_not_logged_in(self):
        ...
        print("\n------ reverse('my-borrowed'):", reverse('my-borrowed'))
    ...
    def test_logged_in_uses_correct_template(self):
        ...
        # or: _ = self.client.login(username='testuser1', password='1X<ISRUkw+tuK')
        # or: self.client.login(username='testuser1', password='1X<ISRUkw+tuK')
        print("\n------ login:", login)
        response = self.client.get(reverse('my-borrowed'))
        print("\n------ response:", response)
        print("\n------ response.context:", response.context)
...
class RenewBookInstancesViewTest(TestCase):
    def setUp(self):
        ...
        # Create a book
        test_author = Author.objects.create(first_name='John', last_name='Smith')
        _ = Genre.objects.create(name='Fantasy')
        test_language = Language.objects.create(name='English')
        test_book = Book.objects.create(
            title='Book Title',
            summary='My book summary',
...
class AuthorCreateViewTest(TestCase):
    ...
    def setUp(self):
        ...
        print("\n------ content_typeAuthor:", content_typeAuthor)
        print("\n------ type(content_typeAuthor):", type(content_typeAuthor))
    ...
    def test_redirect_if_not_logged_in(self):
        print("\n====== test_redirect_if_not_logged_in ======")
        author_create_url = reverse('author-create')
        response = self.client.get(author_create_url)
        print("\n------ response:", response)
        print("\n------ response.context:", response.context)
        redirect_url = f'/accounts/login/?next={author_create_url}'
    ...
    def test_author_create_initial_date_of_death(self):
        print("\n====== test_author_create_initial_date_of_death ======")
        login = self.client.login(username='testuser1', password='1X<ISRUkw+tuK')
        self.assertTrue(login)
        response = self.client.get(reverse('author-create'))
        print("\n------ response:", response)
        print("\n------ response.context:", response.context)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['form'].initial['date_of_death'], '11/11/2123')

    def test_redirects_to_all_authors_list_on_success(self):
        print("\n====== test_redirects_to_all_authors_list_on_success ======")
        login = self.client.login(username='testuser1', password='1X<ISRUkw+tuK')
        self.assertTrue(login)
        response = self.client.post(
